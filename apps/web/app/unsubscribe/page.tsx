    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-[#FF7F50] via-[#FF69B4] to-[#9370DB] text-white">
          <div className="container px-4 py-16 flex flex-col items-center text-center space-y-8">
              <div className="relative">
                  {status === "confirm" && <Mail className="w-16 h-16 md:w-24 md:h-24" />}
                  {status === "processing" && <Loader2 className="w-16 h-16 md:w-24 md:h-24 animate-spin" />}
                  {status === "unsubscribed" && <MailX className="w-16 h-16 md:w-24 md:h-24" />}
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-8">
                  ...
                  {status === "unsubscribed" && (
                      <Link href="/">
                          <Button size="lg" className="bg-white text-pink-600 hover:bg-white/90 font-semibold px-8">
                              Back to Home
                          </Button>
                      </Link>
                  )}
